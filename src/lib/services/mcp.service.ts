...
export class MCPService {
	...
	static createTransport(config: MCPServerConfig): {
		...
		// If URL ends with /sse, use SSE transport directly (skip StreamableHTTP)
		const forceSSE = url.pathname.endsWith('/sse');

		if (forceSSE) {
			if (import.meta.env.DEV) {
				console.log(`[MCPService] Creating SSE transport for ${url.href} (detected /sse endpoint)`);
			}
			return {
