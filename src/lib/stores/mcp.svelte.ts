...
import {
	...
	UrlProtocol,
	JsonSchemaType
	// TODO: Phase 5 - ToolCallType from agentic
	// ToolCallType
} from '$lib/enums';
import {
...
class MCPStore {
	...
	async ensureInitialized(perChatOverrides?: McpServerOverride[]): Promise<boolean> {
		...
		console.log('[MCP] ensureInitialized config:', mcpConfig ? Object.keys(mcpConfig.servers) : 'undefined', 'signature:', !!signature);
		if (!signature) {
			console.log('[MCP] No valid config, shutting down');
			await this.shutdown();
	...
	getToolDefinitionsForLLM(): OpenAIToolDefinition[] {
		...
		for (const connection of this.connections.values()) {
			for (const tool of connection.tools) {
				...
				tools.push({
					type: 'function' as const, // TODO: Phase 5 - use ToolCallType.FUNCTION
					function: {
						name: tool.name,
