 ...
 *
 * Manages conversation lifecycle, persistence, and navigation.
 *
 * **Architecture & Relationships:**
 * - **DatabaseService**: Stateless IndexedDB layer
 * - **conversationsStore** (this): Reactive state + business logic
 * - **chatStore**: Chat-specific state (streaming, loading, message operations)
 *
 ...
 *
 * @see DatabaseService in services/database.service.ts for IndexedDB operations
 */
...
class ConversationsStore {
	...
	isMcpServerEnabledForChat(serverId: string): boolean {
		return this.mcpServerOverrides[serverId] ?? true;
	}

	async toggleMcpServerForChat(serverId: string): Promise<void> {
		const current = this.isMcpServerEnabledForChat(serverId);
		this.setMcpServerOverride(serverId, !current);
	}
}
