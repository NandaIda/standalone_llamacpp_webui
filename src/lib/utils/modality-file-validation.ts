...
export function isFileTypeSupportedByModel(filename: string, mimeType?: string): boolean {
	// For external APIs, allow all file types - let the API decide
	if (isExternalApi()) {
		return true;
	}
...
export function filterFilesByModalities(files: File[]): {
	...
	// For external APIs, allow all files - let the API decide
	if (isExternalApi()) {
		return { supportedFiles: files, unsupportedFiles: [], modalityReasons: {} };
	}
...
export function generateModalityAwareAcceptString(): string {
	...
	// For external APIs, include all file types
	if (isExternalApi()) {
		...
		// For local server, only include if capabilities are available
		const hasVision = supportsVision();
		const hasAudio = supportsAudio();
