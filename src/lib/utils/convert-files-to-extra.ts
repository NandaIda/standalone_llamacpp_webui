	...
	for (const file of files) {
		if (getFileTypeCategory(file.type) === FileTypeCategory.IMAGE) {
			...
			try {
				...
				// For external APIs, trust the user's setting - let the API validate
				// For local servers, force PDF-to-text for non-vision models
				let shouldProcessAsImages = Boolean(currentConfig.pdfAsImage) && (isExternalApi || hasVisionSupport);

				// If user had pdfAsImage enabled but local model doesn't support vision, update setting and notify
				if (currentConfig.pdfAsImage && !hasVisionSupport && !isExternalApi) {
