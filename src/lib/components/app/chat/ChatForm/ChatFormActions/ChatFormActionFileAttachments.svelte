<script lang="ts">
	...
	const allowVision = $derived(isExternalApi || supportsVision());
	const allowAudio = $derived(isExternalApi || supportsAudio());

	const fileUploadTooltipText = $derived.by(() => {
		if (isExternalApi) {
			return 'Attach files - API will validate model capabilities';
		}
