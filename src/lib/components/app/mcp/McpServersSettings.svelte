<script lang="ts">
	...
	// Auto-run health checks for all enabled servers on mount
	onMount(() => {
		const enabledServers = servers.filter((s) => s.enabled && s.url.trim());
		if (enabledServers.length > 0) {
			mcpStore.runHealthChecksForServers(enabledServers);
		}
		// Fallback: force show cards after 10s even if health checks stall
		const timeout = setTimeout(() => { initialLoadComplete = true; }, 10000);
